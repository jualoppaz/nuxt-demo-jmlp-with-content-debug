...
export default {
  ...
  render (h, { data, props }) {
    ...
    if (!body || !body.children || !Array.isArray(body.children)) {
      console.log('==========> Ejecutamos el return vacío en el render de nuxt-content.js')
      return
    }
    data.class = Object.assign({ 'nuxt-content': true }, data.class)
    data.props = Object.assign({ ...body.props }, data.props)
    console.log('==========> Llamamos al processNode');

    return h('div', data, body.children.map((child) => {
      const res = processNode(child, h, document)

      console.log('==========> Salimos del processNode')
      return res
    }))
  }
}
